...
class ImdbSpider(CrawlSpider):
    ...
    IMAGE_DIR = "/Users/Thomas/Desktop/imdb-ptt-scrapy/media/movie/images"

    custom_settings = {
        "IMAGES_STORE": IMAGE_DIR,
        "ITEM_PIPELINES": {
            "imdb_ptt.pipelines.ImdbPttPipeline": 100,
            "imdb_ptt.pipelines.CustomImagePipeline": 1,
        },
    }
    start_urls = [
    ...
    def parse_item(self, response):
        ...
        i["critics_consensus"] = response.xpath(
            "normalize-space(//div[@class='summary_text']/text())"
        ).extract()
        i["date"] = response.xpath("//span[@id='titleYear']/a/text()").extract
        i["duration"] = response.xpath("normalize-space((//time)[1]/text())").extract()
        i["genre"] = response.xpath("//div[@class='subtext']/a[1]/text()").extract()
        # i['rating'] = response.css(
        #     '.ratingValue ::text').extract()[1]
        i["rating"] = response.xpath("//span[@itemprop='ratingValue']/text()").extract()
        amount_reviews = response.xpath("//span[@class='small']/text()").extract()
        i["amount_reviews"] = [str(x.replace(",", "")) for x in amount_reviews]
        url = response.xpath("//div[@class='poster']/a/img/@src").extract()
        link = "".join(url)
